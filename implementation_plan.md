...
**Recent work (scaffolded)**
- Added `expiry_date` and `lot_number` to `products` in `docs/schema.sql` ✅
- Added `suppliers`, `purchase_orders`, `purchase_order_items`, `stock_audits`, and `stock_audit_items` tables ✅
- Updated `docs/seed.sql` to create a sample supplier, PO, and stock audit for testing ✅
- Implemented `src/lib/services/suppliers.ts` for supplier and PO management ✅
- Added client CSV import UI (`/inventory/import`) and server route `/api/import/products` (requires `SUPABASE_SERVICE_ROLE_KEY`) ✅
- Added low-stock checker endpoint `/api/alerts/check-low-stock` (manual run or cron) ✅

> Tip: After applying the DB changes, run `docs/schema.sql` and then `docs/seed.sql` in your Supabase SQL editor. Ensure `SUPABASE_SERVICE_ROLE_KEY` is set for the server import and low-stock endpoints. 🔧
